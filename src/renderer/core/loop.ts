...
function render(timestamp: number, state: RootState) {
  // Run local effects
  let delta = state.ticker.deltaMS;
  // In frameloop='never' mode, clock times are updated using the provided timestamp
  if (state.frameloop === 'never' && typeof timestamp === 'number') {
    delta = timestamp - state.ticker.elapsedMS;
    state.ticker.update(timestamp);
  }
  // Call subscribers (useFrame)
  for (i = 0; i < state.internal.subscribers.length; i++) state.internal.subscribers[i].ref.current(state, delta);
  // Render content
  if (!state.internal.priority && state.gl.render) state.gl.render(state.stage);
